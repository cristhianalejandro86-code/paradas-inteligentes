...
/** Asigna múltiples técnicos (con su cargo) a una tarea. */
export function AsignarTecnicosModal({
  ...
  function guardar() {
    const asignados = usuarios.filter((u) => sel.has(u.id)).map((u) => ({ id: u.id, nombre: u.nombre, rol: u.rol }))
  ...
  const lista = q ? usuarios.filter((u) => u.nombre.toLowerCase().includes(q.toLowerCase())) : usuarios
  ...
  return (
    <div role="dialog" aria-modal="true" onKeyDown={(e) => e.key === 'Escape' && onClose()} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="flex max-h-[90vh] w-full max-w-md flex-col rounded-xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar persona…" className="mb-2 w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm" />
        <div className="grid gap-0.5 overflow-y-auto">
          {lista.map((u) => (
            <label key={u.id} className="flex items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-slate-50">
              ...
              <span className="shrink-0 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">{u.rol.replace('_', ' ')}</span>
            </label>
          ))}
