...
import { planAndamios } from '../lib/andamios'
...
/**
 * Vista ANDAMIOS: con los cuerpos registrados por actividad (Preparación), muestra qué
 * andamios trabajan EN PARALELO (ventanas que se pisan → cuerpos distintos) y cuáles
 * EN SERIE (se desarma, traslada y rearma el mismo cuerpo). Con las horas de mudanza
 * configurables calcula el MÍNIMO de cuerpos a conseguir y el plan de reutilización.
 */
export function AndamiosPage() {
  ...
  const [mudanzaH, setMudanzaH] = useState(() => { const s = localStorage.getItem('andamios-mudanza-h'); const n = s ? Number(s) : NaN; return Number.isFinite(n) && n >= 0 ? n : 4 })
  useEffect(() => { try { localStorage.setItem('andamios-mudanza-h', String(mudanzaH)) } catch { /* sin persistencia */ } }, [mudanzaH])
  ...
  const plan = useMemo(() => planAndamios(scoped, mudanzaH), [scoped, mudanzaH])

  // etiqueta de cuerpo por tarea (para colorear la serie: mismo color = mismo andamio físico)
  const cuerpoDe = useMemo(() => {
    const m: Record<string, string[]> = {}
    for (const u of plan.cuerpos) for (const c of u.cadena) (m[c.t.id] ??= []).push(u.etiqueta.replace('Cuerpo ', 'A'))
    return m
  }, [plan])

  if (loading) return <p className="text-sm text-slate-400">Cargando andamios…</p>
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="flex items-center gap-2 rounded-lg border border-orange-300 bg-orange-50 px-3 py-1.5 text-sm text-orange-800" title="Horas que toma desarmar un andamio, trasladarlo y rearmarlo en el siguiente punto. Afecta cuántos usos en serie caben.">
          ⏱ Desarme + traslado + armado:
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi label="Actividades con andamio" value={String(plan.acts.length)} sub={plan.sinFecha.length ? `+ ${plan.sinFecha.length} sin programar` : 'todas programadas'} />
        <Kpi label="Sin reutilizar (suma)" value={`${plan.totalSinReusar}`} sub="cuerpos si cada actividad tuviera el suyo" />
        <Kpi label="Pico en paralelo" value={`${plan.pico}`} sub="cuerpos parados a la vez (mínimo físico)" accent="text-amber-600" />
        <Kpi label={`NECESITAS (con mudanza ${mudanzaH}h)`} value={`${plan.necesarios}`} sub={ahorro > 0 ? `reutilizando ahorras ${ahorro} cuerpo(s)` : 'sin reutilización posible'} accent="text-orange-600" />
      </section>

      {plan.acts.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-400">
          Aún no hay actividades con andamios programadas.<br />
          Regístralos en <b>Preparación → columna «Andamios (cuerpos)»</b> y esta vista arma el plan sola.
        </div>
      ) : (
        <>
          ...
          <div className="rounded-xl border border-slate-200 bg-white">
            <div className="border-b border-slate-100 px-4 py-3">
              <h3 className="text-sm font-semibold text-slate-700">Cronograma de andamios <span className="ml-2 text-xs font-normal text-slate-400">barras que se PISAN = cuerpos en paralelo · mismo color = el MISMO cuerpo reutilizado (serie)</span></h3>
            </div>
              ...
              <div className="relative" style={{ width: 300 + timelineW, minWidth: '100%' }}>
                ...
                <div className="flex border-t-2 border-slate-300 bg-slate-50/60" style={{ height: 56 }}>
                  <div className="sticky left-0 z-10 flex shrink-0 flex-col justify-center border-r border-slate-200 bg-slate-50 px-2 text-[10px] font-semibold uppercase text-slate-500" style={{ width: 300 }}>
                    Cuerpos ocupados / hora
                    <span className="normal-case text-amber-600">pico {plan.pico}</span>
                  </div>
                  <div className="relative shrink-0" style={{ width: timelineW }}>
                    {plan.histo.map((c, h) => c > 0 ? (
                      <div key={h} className="absolute bottom-2" style={{ left: h * hourW, width: Math.max(hourW - 1, 2) }} title={`${c} cuerpo(s)`}>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <h3 className="mb-1 text-sm font-semibold text-slate-700">🔁 Plan de reutilización — {plan.necesarios} cuerpo(s) físico(s)</h3>
            <p className="mb-3 text-[11px] text-slate-400">Cada fila es UN andamio real: se arma para la primera actividad y, al terminar, se desarma, traslada y rearma para la siguiente (≥ {mudanzaH} h entre usos).</p>
                ...
                <div key={u.etiqueta} className="flex flex-wrap items-center gap-1.5 rounded-lg border border-slate-100 bg-slate-50/60 p-2 text-xs">
                  ...
                  {u.cadena.map((c, i) => (
                    <span key={i} className="flex items-center gap-1.5">
                      {i > 0 && <span className="text-orange-500" title={`desarmar + trasladar + rearmar (${mudanzaH} h)`}>→ 🚚 →</span>}
          ...
          {plan.sinFecha.length > 0 && (
            <div className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-700">
              <b>📋 {plan.sinFecha.length} actividad(es) con andamio SIN programar</b> — no entran al cálculo hasta tener Comienzo/Fin:
              <ul className="mt-1 list-inside list-disc text-xs">
                {plan.sinFecha.map((a) => <li key={a.t.id}>#{a.t.secuencia} {a.t.nombre} · {a.c} cuerpo(s)</li>)}
              </ul>
            </div>
          )}
        </>
